...
**Versão v1.4.11** — Assistente IA com renderização Markdown no chat. Adicionada a função `markdownToHtml()` inline em `app/templates/assistente/index.html` (~60 linhas de regex vanilla JS) que converte `**negrito**`, `*itálico*`, `# headers`, `- listas`, ```bloco de código```, `` `inline` ``, `> citações` e links `[texto](url)` em HTML antes de `wrapper.innerHTML`. Fluxo: `escaparHtml(texto)` protege contra XSS → `markdownToHtml()` interpreta a sintaxe. Regras CSS adicionadas em `app/static/css/pipe.css` para `.chat-bubble strong`, `em`, `code`, `pre`, `ul`, `li`, `blockquote`, `h3/h4/h5`, `a` — respeitam tokens de tema claro/escuro. Sem alteração de BD.
